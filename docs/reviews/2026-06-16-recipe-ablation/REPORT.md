...
- **Eval:** `scripts/predict.py --model_path <ckpt> --split_file <test split, val=129 FOVs> --ct_abstention_k 0`, then `hierarchical_macro_f1` over the prediction CSV. Loading a `resmlp` checkpoint requires the head-agnostic cell-type-count inference added in `b598710` (a `xuefei/master` commit, not in PR #41).
