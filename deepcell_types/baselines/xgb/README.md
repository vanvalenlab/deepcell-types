...
- **Class balancing — DCT sampler (default).** All methods now share the main
  DeepCell-Types model's sampler. For XGBoost (a tree model, no sampler) this is
  applied as a per-row `sample_weight` in `fit()` using the DCT sampler weights
  (sqrt-inverse-frequency with a 1000-count floor); `--class_balance dct` is the
  default, `--class_balance none` restores the faithful unweighted XGBoost. This
  removes the prior asymmetry where XGBoost ran unbalanced while MAPS/CellSighter
  were balanced — which had made the XGBoost rare-class macro number conservative.
